...
const sampleProducts = [
  ...
  },
  {
    ...
    price: 180000,
    description:
      "Ish va kundalik hayot uchun ideal klassik shim. Yuqori sifatli material va zamonaviy dizayn.",
    category: "kiyim",
    badge: "Mashhur",
    images: [
    ...
    rating: 4.2,
    reviews: [
      { name: "Bobur", rating: 4, text: "Yaxshi sifat, narxi ham mos." },
    ],
  },
  {
    id: 3,
  ...
  },
  {
    ...
    price: 320000,
    description:
      "Qulay va zamonaviy sport krossovka. Sport va kundalik hayot uchun ideal tanlov.",
    category: "yangi",
    badge: "Yangi",
    images: [
      "https://images.pexels.com/photos/1536619/pexels-photo-1536619.jpeg?auto=compress&cs=tinysrgb&w=400",
  ...
  },
  {
    ...
    price: 200000,
    description:
      "Yengil va salqin yoz libosi. Issiq kunlar uchun ideal tanlov. Nafas oladigan material.",
    category: "kiyim",
    badge: "Limited",
    images: [
      "https://images.pexels.com/photos/1884581/pexels-photo-1884581.jpeg?auto=compress&cs=tinysrgb&w=400",
    ...
    rating: 4.3,
    reviews: [
      { name: "Nigora", rating: 4, text: "Chiroyli libos, lekin tez tugaydi." },
    ],
  },
  {
    id: 6,
    ...
    price: 450000,
    description:
      "Zamonaviy va elegant soat. Har qanday kiyim bilan mos keladi. Suv o'tkazmaydigan va bardoshli.",
    category: "aksessuarlar",
    badge: "Premium",
    images: [
      "https://images.pexels.com/photos/1040945/pexels-photo-1040945.jpeg?auto=compress&cs=tinysrgb&w=400",
    ...
    searchResults.innerHTML = `
            <div class="search-results-list">
                ...
                ${results
                  .map(
                    (product) => `
                    <div class="search-result-item" onclick="openProductModal(${
                      product.id
                    }); toggleSearch();">
...
function filterByPrice() {
  ...
  const min = parseInt(minPrice.value) * 1000;
...
function clearFilters() {
  ...
  document
    .querySelector(".color-option[data-color='all']")
    .classList.add("active");
...
function pauseSlider() {
  ...
  if (sliderPaused) {
    ...
    showNotification("Slayder to'xtatildi", "info");
  } else {
    pauseBtn.className = "fas fa-pause";
    showNotification("Slayder davom ettirildi", "info");
  }
}
...
function createProductCard(product) {
  ...
  card.innerHTML = `
    ...
    product.name
            ...
            ${
              product.badge
                ? `<div class="product-badge ${product.badge.toLowerCase()}">${
                    product.badge
                  }</div>`
                : ""
            }
            ${
              !product.inStock
                ? '<div class="product-badge out-of-stock">Tugagan</div>'
                : ""
            }
        </div>
...
function openProductModal(productId) {
  ...
  if (product.oldPrice) {
    document.getElementById("modalProductOldPrice").textContent = formatPrice(
      product.oldPrice
    );
...
function handleReviewSubmit(e) {
  ...
  const text =
    formData.get("review") || e.target.querySelector("textarea").value;
  const ratingStars = e.target.querySelectorAll(
  ...
  const productIndex = products.findIndex(
    (p) => p.id === currentProductModal.id
  );
  if (productIndex !== -1) {
    products[productIndex].reviews.push(newReview);

    // Update average rating
    const totalRating = products[productIndex].reviews.reduce(
      (sum, r) => sum + r.rating,
      0
    );
    products[productIndex].rating =
      totalRating / products[productIndex].reviews.length;
...
function toggleCart(force = null) {
  ...
  if (
    force === true ||
    (!sidebar.classList.contains("active") && force !== false)
  ) {
...
function addToCart() {
  ...
  const selectedColor =
    document.querySelector(".color-option.active")?.style.background ||
    "#000000";
  const selectedSize =
    document.querySelector(".size-option.active")?.textContent || "M";

  // Check if item already exists in cart
  const existingItem = cart.find(
    (item) =>
      item.productId === currentProductModal.id &&
            ...
            <div class="item-details">
                ...
                <div class="quantity-controls">
                    <button onclick="updateQuantity(${
                      item.id
                    }, -1)" title="Kamaytirish">-</button>
                    <span>${item.quantity}</span>
                    <button onclick="updateQuantity(${
                      item.id
                    }, 1)" title="Ko'paytirish">+</button>
                    <button onclick="removeFromCart(${
                      item.id
                    })" class="remove-btn" title="O'chirish">
...
function showOrderModal() {
  ...
  orderItems.innerHTML = cart
    ...
    <div class="order-item">
      <span>${item.name} (${item.size}, ${item.colorName}) x${
        item.quantity
      }</span>
...
function handleOrderSubmit(e) {
  ...
  showNotification(
    "Buyurtma muvaffaqiyatli berildi! Tez orada aloqaga chiqamiz.",
    "success"
  );
...
function toggleSearch(force = null) {
  ...
  if (
    force === true ||
    (!searchOverlay.classList.contains("active") && force !== false)
  ) {
...
function toggleTheme() {
  ...
  showNotification(
    `${currentTheme === "dark" ? "Tungi" : "Kunduzgi"} rejimga o'tkazildi`,
    "info"
  );
}
...
function handleAddProduct(e) {
  ...
  const images = Array.from(document.querySelectorAll(".preview-image")).map(
    (img) => img.src
  );
...
function editOrder(orderId) {
  showNotification("Buyurtmani tahrirlash funksiyasi tez orada!", "info");
}
